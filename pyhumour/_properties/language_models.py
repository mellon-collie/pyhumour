...
class NgramHelper:
	def __init__(self,texts : list):
		...
		self._bigram = dict()
		self._trigram = dict()
		self._special_characters = string.punctuation + "'[@_!#$%^&*()<>?/\|}{~:]'"
			...
			if len(word_list) >= 3:
				for j in range(0, len(word_list)-2):
					if (word_list[j], word_list[j+1], word_list[j+2]) not in self._trigram:
						self._trigram[(word_list[j], word_list[j+1], word_list[j+2])] = 1
					else:
						self._trigram[(word_list[j], word_list[j+1], word_list[j+2])] += 1
			...
			if len(word_list) > 0:
				for j in range(len(word_list) - 2):
					if (word_list[j], word_list[j + 1], word_list[j + 2]) not in self._trigram:
						...
						probability += math.log(self._trigram[(word_list[j], word_list[j + 1], word_list[j + 2])])
